...
module.exports = function formatOrder(order) {
  const FN1 = [
    'FN1',                           // FN1_RecType (3)
    pad(order.id, 20, true),        // FN1_OrderNo (20)
    pad(order.customer?.id, 20),    // FN1_DelCustNo
    pad(order.customer?.name, 50),  // FN1_DelName
  ...
  const FN2 = order.line_items.map(item => [
    'FN2',                                   // FN2_RecType
    pad(order.id, 20, true),                // FN2_OrderNo
